...
- 写入代码、文档或界面文案的中文内容应使用正常中文标点；用户输入中因输入法产生的英文逗号、句号等，不应原样沿用到产出物中。
- 不主动读取或输出 `.env`、`secrets/`、credentials、密钥、token 等敏感文件；除非用户明确授权且任务必要。
- 不要自行启动项目工程或本地常驻服务；可以认为项目默认已经由人工启动且具备热更新。需要运行态验证时，交由人工帮忙校验，AI 只运行 build、lint、typecheck、test 等一次性验证命令。
