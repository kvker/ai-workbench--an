...
|----|----------|----------|--------------|------|
| 前端登录态 | `repos/app/src/services/auth.ts` 已接入 devops 登录并保存 token；`repos/app/src/services/session.ts` 中 `mockSession.userName = zweizhao` 仍用于工作区 service 的 `x-workspace-user` 兜底 | devops 请求使用真实 token；工作区 service 请求仍带前端声明的用户名 | service 校验登录 token 或 session，从可信身份中解析用户标识；前端不再直接声明工作区用户 | 部分替换 |
