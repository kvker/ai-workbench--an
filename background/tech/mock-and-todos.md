...
|----|----------|----------|--------------|------|
| 前端登录态 | `repos/app/src/services/auth.ts` 已接入 SSO 回调：`/auth/sign-in` 调用 service `/api/mock/sso`，保存 qingtian 用户到 `ai-workbench:login-user-qingtian`，保存 devops 登录态到 `ai-workbench:login-user`；`repos/app/src/services/session.ts` 中 `mockSession.userName = zweizhao` 仍用于工作区 service 的 `x-workspace-user` 兜底 | devops 请求使用当前登录 token；工作区 service 请求仍带前端声明的用户名 | service 校验登录 token 或 session，从可信身份中解析用户标识；前端不再直接声明工作区用户 | 部分替换 |
| SSO devops 映射 mock | `repos/service/src/routes/sso.js` 真实调用 qingtian `/sso/userInfo` 获取 SSO 用户；`repos/service/src/routes/mock.js` 提供临时 `/api/mock/sso`，仅将 qingtian 用户构造为 devops 登录态形状，并透传 qingtian 返回的真实 token | SSO 回调登录；当前 DevOps 登录态映射仍是临时接口，但 token 来自真实 SSO 返回 | 替换为真实 devops SSO 映射接口，按 qingtian 手机号匹配本地用户并发放真实 token；保留 appSecret 仅服务端可见 | 待替换 |
