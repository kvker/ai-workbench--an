...
根据 `repos/` 中的实际代码和 `artifacts/` 中的已完成工作，反向更新 `background/` 文档。
...
```bash
node .agents/skills/an-refresh/scripts/scan-repos.mjs --root repos --artifacts artifacts --write artifacts/background-scan.json
```

1. 读取 `background/` 下所有文档，了解当前记录的状态
2. 扫描 `repos/` 目录，提取实际信息：
...
3. [structure.md] 新增了 repos/apps/notification/ 目录未记录
...
| repos/ 代码无法解析 | 跳过自动分析，提示用户手动提供信息 |
