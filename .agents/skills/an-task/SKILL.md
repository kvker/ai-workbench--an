---
name: an-task
description: 根据输入的任务描述，先智能判断任务复杂度与风险，再选择 L0 直接处理、L1 快速修复、L2 标准开发或 L3 完整流程。适用于开发任务、bug 修复、代码重构、配置修改、文档更新、需求实现等。当用户想要：1) 开始一个开发任务 2) 提供了任务/票据/想法的原始描述 3) 执行 /an-task 命令 4) 提到"开发"、"实现"、"修复"、"添加"等关键词时触发此技能。触发后不要默认套完整流程；小改动可不创建 artifacts，直接实现、验证并总结。
---

# AI Native 任务开发

将任务描述（票据、想法、问题描述）按最轻但足够安全的 AI Native 流程推进到代码实现。
...
| 等级 | 使用方式 | 何时使用 |
|------|----------|----------|
| L0 直接处理 | 不进入流程，不创建 artifacts；直接改、验证、总结 | 改字、改注释、明显一行修复、无行为风险的小配置 |
| L1 快速修复 | 可不创建 artifacts；必要时只保留轻量记录 | 单模块、小范围、目标明确、有最小验证 |
| L2 标准开发 | 创建 artifacts，从 raw-input + tech-spec 开始 | 明确的小功能、方案清楚、影响少量模块 |
| L3 完整流程 | 创建 artifacts，按全阶段推进并确认 | 需求模糊、跨模块、架构/接口/数据/权限等高风险改动 |

执行要求：

1. 先用一句话告诉用户采用的等级和理由；默认由 AI 自主决策并推进，不把可由 AI 判断的流程等级、技术方案或验证命令交给用户选择。
2. 选择能覆盖风险的最轻流程。不要因为技能被触发就机械创建 artifacts 或跑完整阶段。
3. 若调查中发现风险升高，立即升级流程并说明原因。
4. 用户明确要求“快速处理”“直接改”时，可在不违反风险条件的前提下降级；用户明确要求“走完整流程”时按用户要求执行。
5. 只有需求目标不明确、验收标准无法自洽、涉及权限/安全/数据/部署等高风险边界、需要破坏性操作或外部授权时，才暂停向用户确认。
...
根据输入和初步代码调查判断工作流范围。优先套用 `conventions/flow-policy.md`，本节只保留快速判断口径：

| 信号 | 范围 | 行为 |
|------|------|------|
| 拼写、注释、明显一行 bug、无行为风险的小配置 | L0 直接修复 | 不创建 artifacts，不进入阶段流程；直接改并验证 |
...
示例：修正文案错别字、改 Markdown 链接、补一行注释、修明显 typo。
...
1. L2/L3 每个阶段在 `artifacts/{YYYYMMDD}__{feature-name}/` 对应子目录下产出文件（日期为创建当天的 `YYYYMMDD` 格式，如 `20260402__user-login`）
2. L2/L3 每阶段结束：展示产出物、AI 决策、取舍理由和风险 → 更新产出目录 AGENTS.md 索引 → 若无暂停条件则继续下一阶段；不要询问用户“选择哪个方案”或“是否进入下一阶段”
...
**检查点**：向用户展示需求文档、AI 提炼的 AC 和范围；若存在无法由代码事实或用户原始输入解决的开放问题，则暂停确认，否则继续下一阶段。

---

### 阶段三：design（技术选型）

**目标**：AI 自主比较技术方案并做出决策。不产出施工细节。
...
**检查点**：向用户展示技术选型、最终选择、未选方案和取舍理由；不要要求用户在方案间选择。仅当决策依赖业务偏好、外部授权或高风险边界时暂停确认，否则继续下一阶段。
...
**检查点**：向用户展示技术规范、变更清单和测试计划；若无开放问题或高风险操作，直接进入实现。
...
**检查点**：向用户展示测试结果；若存在失败、缺陷或未关闭风险，AI 先自行修复和复测，只有无法继续时才请求用户决策。
