...
| 临时目录脚本调用 `replaceHarnessAgentFiles` | 通过 | 验证 `frontend/fe`、`backend/be`、`product/pm`、`test/qa` 与 `shared/delivery/engineering` 均写入预期的 `agents/*.md` 或 `agents/shared/*.md`，且优先使用映射中的第一个存在源。 |
