...
| 复杂度统计 | `repos/app/src/services/report.ts` 不再基于 `ops_issue.issue_type` 映射 L/M/S/XS，改为按真实周期分组 |
...
| 文档同步 | 更新前端命名规范，并在 mock-and-todos 登记效能看板真实数据统计口径、7/3/1 天分组和 `updated_at` 归档时间口径 |
...
- 复杂度口径不再来自 `issue_type`；按真实周期分为 L、M、S、XS：L 为 7 天以上，M 为 3 天以上，S 为 1 天以上，XS 为 1 天以内。
- 复杂度周期分布按已归档 Harness 需求分别计算 P85、P50、平均值，直接使用真实周期；无真实周期样本时显示为空；页面统一使用“天”作为单位，并固定按 L、M、S、XS 顺序展示。
