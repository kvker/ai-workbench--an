...
| 临时目录脚本调用 `replaceHarnessAgentFiles` | 通过 | 使用真实 `/Users/zweizhao/project/current/qingtian-harness`，生成 `product.toml`、`qa-testcase.toml`、`qa.toml`、`shared.toml`。 |
...
| 清理旧 agents 映射后重新调用 `replaceHarnessAgentFiles` | 通过 | 仍生成 4 个 TOML 文件，`qa-testcase.toml` 头部保持 `name/description/developer_instructions`。 |
